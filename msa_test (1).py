...
class Alignment(object):
    ...
    def __init__(self, file_1, file_2):
        #assuming 2000bp were allowed on either side of organism's DmdA 
        self.organism1, self.seq1 = self.read_fasta(file_1)
        self.organism2, self.seq2 = self.read_fasta(file_2)
        self.seq1_left = self.seq1[:2000]
        self.seq1_right = self.seq1[-2000:]
        self.seq2_left = self.seq2[:2000]
        self.seq2_right = self.seq2[-2000:]
    ...
    def traceback(self, organism1, seq1, d1, organism2, seq2, d2, F, TB):
        ...
        while iterations < max_iter:
            ...
            #region in that 2000bp flanking region
    ...
    def get_conserved(self, organism1, organism2, seq1_left, seq1_right, seq2_left, seq2_right):
        ...
        #with 2000bp on both sides. l - 4000 = length of DmdA. so beginning of conserved
        #regions coming from right flank start at l - 4000 + 2000, or l - 2000 from the
        ...
        for i in self.traceback(organism1, seq1_left, 0, organism2, seq2_right, len(self.seq2)-2000, F, TB):
            conserved.append(i)
        F, TB = self.Smith_Waterman(seq1_right, seq2_left)
        for i in self.traceback(organism1, seq1_right, len(self.seq1)-2000, organism2, seq2_left, 0, F, TB):
            conserved.append(i)
        F, TB = self.Smith_Waterman(seq1_right, seq2_right)
        for i in self.traceback(organism1, seq1_right, len(self.seq1)-2000, organism2, seq2_right, len(self.seq2)-2000, F, TB):
            conserved.append(i)
        return conserved
